...
- **Offline-first**: IndexedDB per-user database
- **Cloud sync**: Background Supabase sync with conflict resolution
- **Realtime**: Supabase Realtime subscription for cross-device updates
- **Migration**: Automatic localStorage → IndexedDB migration on first load
- **Encryption**: API keys are XOR-obfuscated locally
...
- **離線優先**：每個使用者獨立 IndexedDB 資料庫
- **雲端同步**：背景 Supabase 同步含衝突解決機制
- **即時更新**：Supabase Realtime 訂閱，跨裝置同步
- **自動遷移**：首次載入時 localStorage → IndexedDB 遷移
- **加密**：API key 本地 XOR 混淆
